...
interface KitchenProps {
  onPrepareFood: (food: FoodType) => void;
  foodTray: FoodType[];
  maxSlots: number;
}

const Kitchen: React.FC<KitchenProps> = ({ onPrepareFood, foodTray, maxSlots }) => {
  const [cooking, setCooking] = useState<Record<FoodType, number>>({
    [FoodType.TTEOKBOKKI]: 0,
    [FoodType.KIMBAP]: 0,
    [FoodType.RAMEN]: 0,
    [FoodType.TWIGIM]: 0,
    [FoodType.SODA]: 0,
  ...
  const startCooking = (foodType: FoodType) => {
  ...
  return (
    <div className="flex flex-col h-full gap-3 overflow-hidden">
      ...
      <div className="flex flex-col gap-2.5 flex-grow overflow-y-auto pr-1 scrollbar-hide">
        {(Object.keys(FOOD_DATA) as FoodType[]).map((type) => {
